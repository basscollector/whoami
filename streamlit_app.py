...
def load_data():
    try:
        ...
        # 2. Macierze Treści (BETA FIX: Czyścimy klucze przy ładowaniu)
...
if submit and DB is not None:
    ...
    # --- MATCHING ZAWODÓW ---
    st.divider()
    st.subheader("🎯 Rekomendowane Ścieżki")
    ...
    if DF_JOBS_TAGGED.empty:
        ...
        if recommendations.empty:
            ...
            for i, (idx, job) in enumerate(recommendations.iterrows()):
                with st.expander(f"#{i+1}: {job['Nazwa']} ({job['Kod_RIASEC']})", expanded=True):
                    st.markdown(f"**Opis:** {job['Opis']}")
